...
def check_status_code_http(
        expected_status_code: requests.codes = requests.codes.OK, expected_title: str = "", expected_errors: dict = None
):
    exp_errors = expected_errors or {}
    try:
        yield
        ...
        if expected_errors:
            raise AssertionError(f'Должны быть получены ошибки "{exp_errors}", но запрос прошел успешно')
        ...
        assert actual_title == expected_title, f'Ожидалось сообщение "{expected_title}", а получили "{actual_title}"'
        if expected_errors:
